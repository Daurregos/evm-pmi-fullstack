...
- [x] 1.1 Añadir `recharts@2.15.4` como dependencia exacta de producción y
  comprobar con una prueba desechable que `renderToStaticMarkup` produce el SVG
  completo, ejes y leyenda incluidos.
- [x] 1.2 Añadir `tests/**/*.tsx` a `include` de `tsconfig.json` y ampliar el
...
- [x] 1.3 Añadir `tests/client/simulated-api.ts`: una implementación de `fetch`
...
- [x] 2.1 Escribir primero `tests/client/evm-api-client.test.ts` contra la API
...
- [x] 2.2 Escribir primero `tests/client/format.test.ts` con montos del fixture,
...
- [x] 3.1 Escribir primero `tests/client/index-badge.test.tsx`: los cuatro
...
- [x] 4.1 Escribir primero `tests/client/activities-table.test.tsx` sobre las
...
- [x] 4.2 Escribir primero `tests/client/project-summary.test.tsx` sobre
...
- [x] 4.3 Escribir primero `tests/client/activities-chart.test.tsx` —tres series
...
- [x] 5.1 Escribir primero `tests/client/dashboard.test.tsx` sobre la
  composición, el selector y `emptyProject`, observar RED e implementar
  `src/ui/dashboard.tsx`, `src/ui/project-selector.tsx` y el aviso de proyecto
  sin actividades.
- [x] 5.2 Escribir primero `tests/client/dashboard-data.test.tsx` —carga inicial
  contra la API simulada y carga centinela renderizada tal cual—, observar RED e
  implementar `src/ui/dashboard-data.ts`.
- [x] 5.3 Implementar `src/ui/dashboard-view.tsx`, `src/app/layout.tsx`,
...
- [x] 6.1 Escribir primero `tests/client/client-boundaries.test.ts`: ningún
...
- [x] 7.1 Documentar en `README.md` la deuda de Recharts 2.x, la omisión de
  «avance anticipado» y cómo levantar el dashboard contra el mock.
- [x] 7.2 Ejecutar de forma fresca `npm run lint`, `npm run lint:imports`,
  `npm run typecheck`, `npm run test:client`, `npm run test:structure`,
  `npm run build` y `git diff --check`, leyendo sus salidas completas.
- [x] 7.3 Contrastar el diff con `docs/TESTING.md` y con la sección
