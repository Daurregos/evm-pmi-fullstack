...
The repository SHALL contain a concise docs/ARCHITECTURE.md that describes only
the implemented layer boundaries, a GET /projects/{id} end-to-end journey, the
unique EVM calculation boundary and a routing table to the ten accepted ADRs.
It SHALL state that real HTTP handlers are pending in A2 and that the currently
executable HTTP surface is /mock-api.

#### Scenario: Architecture reader identifies active and pending boundaries
- **WHEN** a reader opens docs/ARCHITECTURE.md
- **THEN** they can identify the dependency rules for domain, application,
  infrastructure, HTTP/mock and UI, the location of EVM calculation, the ten
  ADR references, and the A2 limitation without inferring components that do
  not yet exist
